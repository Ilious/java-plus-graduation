package ru.practicum.listener.base;
...
@Slf4j
public abstract class BaseProcessor<E,T extends SpecificRecordBase> {
    ...
    protected abstract void handleRecord(ConsumerRecord<E, T> record);

    protected void manageOffset(ConsumerRecord<E, T> record, int count, Consumer<E, T> consumer) {
        ...
        if (count % batchMessagesCount == 0) {
            consumer.commitAsync(currentOffsets, (offset, ex) -> {
                if (ex != null) {
                    log.warn("Error during fixing offsets: {}", currentOffsets, ex);
                }
            });
        }
    }
